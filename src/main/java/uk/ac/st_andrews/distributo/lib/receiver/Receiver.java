...
public class Receiver implements Runnable {
    ...
    public Receiver(String groupHost, int groupPort, String controlHost, int controlPort) throws IOException {
    ...
    private void acceptData() throws IOException {
        ...
        while (acceptingData) try {
            ...
            //Packet p = Packet.fromStream(bais);
            Packet p = new Packet();
            p.unmarshal(buffer);
            System.out.printf("[%s]: <%s> \"%s\"\n",
                    datagramPacket.getAddress().toString(),
                    p.toString(),
                    p.data() == null ? "null" : new String(p.data())
            );
